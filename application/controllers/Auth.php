...
class Auth extends CI_Controller {
    ...
    function login_ibama($errorMsg = NULL) {
        ...
        if (!$this->authldap->is_authenticated()) {
            ...
            $_POST['inputUsername'] = str_replace(array('-','/','.'), '', $_POST['inputUsername']);
    ...
    function login_empresa($errorMsg = NULL) {
        ...
        if (!$this->session->userdata('logged_in')) {
            ...
            $this->firephp->log(strlen($_POST['inputUsername']));

            // Selecting the num_pessoa from the cnt database

            if ($_POST['inputUsername'] && (strlen($_POST['inputUsername']) == 18)) {
                ...
                $sql = 'begin
                            :erro := sisreg.pk_autenticar.autenticar( p_cpf_cnpj_num_pessoa => :p_cpf_cnpj_num_pessoa
                                                                       ,p_senha             => :p_senha
                                                                       ,p_num_pessoa        => :p_num_pessoa
                                                                       ,p_cod_tipo_pessoa   => :p_cod_tipo_pessoa
                                                                       ,p_cpf_cnpj          => :p_cpf_cnpj
                ...
                $cpf_cnpj_num_pessoa = str_replace(array('-','/','.'), '', $_POST['inputUsername']);
                $p_num_pessoa = null;  // Our app_modulo squence
                $p_cod_tipo_pessoa = null;  // Our app_modulo squence
                $p_cpf_cnpj = null;  // Our app_modulo squence
                $p_seq_app_modulo = null;  // Our app_modulo squence
                // Data Binding in variables
                oci_bind_by_name($p_str, ":p_cpf_cnpj_num_pessoa", $cpf_cnpj_num_pessoa);
                oci_bind_by_name($p_str, ":p_senha", $p_senha);

                // Data Binding out variables
                oci_bind_by_name($p_str, ':p_num_pessoa', $p_num_pessoa, 25);
                oci_bind_by_name($p_str, ':p_cod_tipo_pessoa', $p_cod_tipo_pessoa, 25);
                oci_bind_by_name($p_str, ':p_cpf_cnpj', $p_cpf_cnpj, 25);
                ...
                // $this->firephp->log($p_cod_tipo_pessoa);


                if ((int)$p_num_pessoa > 0) {
                    ...
                    oci_bind_by_name($p_str, ":p_cpf_cnpj_nome", $username, 20);
                    ...
                    $this->load->view('pages/login_empresa', array('login_fail_msg'
                        => 'No user matched this CPF/CNPJ number.'));
                }

                // CODE FOR ACCESSING CNT DATABASE
            } else {
                if ($_POST['inputUsername'].length != 15) {
                    $this->load->view('pages/login_empresa', array('login_fail_msg'
                        => 'Insert a valid CNPJ.'));
                } else {
                    // Login FAIL
