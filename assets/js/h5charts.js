...
(function() {
  var bool, chart1, chart2, chart5, chart7, chart8, i, knob1, knob2, knob3, opt, rest, spark1, spark2, _i, _ref,
...
console.log('AQUI JADSON!!!!');
  ...
  for (i = _i = _ref = H5.Data.thisYear; _ref <= 2004 ? _i <= 2004 : _i >= 2004; i = _ref <= 2004 ? ++_i : --_i) {
  ...
  H5.DB.occurence.data = {
    init: function() {
      var region, _j, _len, _ref1;
      this.regions = {};
      _ref1 = H5.Data.regions;
  ...
  chart1.drawChart = function() {
    var contMes, createTable, data, day, daysInMonth, firstPeriod, indexMes, mes, mesIndex, monthLoop, months, options, secondPeriod, titleChart, titleX, todosMeses, _j, _k, _l, _len;
    createTable = (function(_this) {
      return function(region, type, origin, indexMonth) {
        var day, daysInMonth, firstPeriod, secondPeriod, sum, _j;
        sum = 0;
        ...
        for (day = _j = 1; 1 <= daysInMonth ? _j <= daysInMonth : _j >= daysInMonth; day = 1 <= daysInMonth ? ++_j : --_j) {
          $.each(H5.DB.occurence.data.regions[region], function(key, reg) {
            ...
            } else if ((firstPeriod <= (_ref2 = reg.date) && _ref2 <= secondPeriod) && reg.day === day && (reg.type.indexOf(type) >= 0 || type === "Todos") && (reg.origin.indexOf(origin) >= 0 || origin === "Todos")) {
              return sum++;
            }
          });
          if (!todosMeses) {
    ...
    contMes = 1;
    indexMes = 0;
    for (_k = 0, _len = monthLoop.length; _k < _len; _k++) {
      mesIndex = monthLoop[_k];
      ...
      if (todosMeses) {
        ...
        for (day = _l = 1; 1 <= daysInMonth ? _l <= daysInMonth : _l >= daysInMonth; day = 1 <= daysInMonth ? ++_l : --_l) {
          data[0] = day;
          data[1] = 0;
          this.data.addRow(data);
        }
  ...
  chart2.drawChart = function() {
    var data, month, options, sumValues, _j, _k, _ref1, _ref2;
      ...
      if (H5.Data.region === "Todos") {
        $.each(H5.DB.occurence.data.regions, function(key, region) {
          return $.each(region, function(key, reg) {
            ...
            } else if ((firstPeriod <= (_ref2 = reg.date) && _ref2 <= secondPeriod) && reg.month === month && (reg.type.indexOf(type) >= 0 || type === "Todos") && (reg.origin.indexOf(origin) >= 0 || origin === "Todos")) {
              return sum++;
            }
          });
        });
  ...
  chart5.drawChart = function() {
    var allData, data, institution, j, options, sumValues, totalReg, _j, _k, _l, _len, _ref1, _ref2, _ref3;
    sumValues = function(institution, region, year, type, origin) {
      ...
      secondPeriod = new Date(year, 11, 31);
      $.each(H5.DB.occurence.data.regions[region], function(key, reg) {
        var t, _j, _k, _len, _len1, _ref1, _ref2, _ref3, _ref4, _results, _results1;
        if (reg.institution !== null) {
          if (type === "Todos" && origin === "Todos") {
            if ((firstPeriod <= (_ref1 = reg.date) && _ref1 <= secondPeriod)) {
              _ref2 = reg.institution.split(",");
              _results = [];
          ...
          } else if ((firstPeriod <= (_ref3 = reg.date) && _ref3 <= secondPeriod) && (reg.type.indexOf(type) >= 0 || type === "Todos") && (reg.origin.indexOf(origin) >= 0 || origin === "Todos")) {
            _ref4 = reg.institution.split(",");
            _results1 = [];
  ...
  chart7.drawChart = function() {
    var data, options, originTitle, region, sumValues, _j, _ref1;
    ...
    this.data.addColumn("number", H5.Data.selectedYear);
    for (i = _j = 0, _ref1 = H5.Data.regions.length; 0 <= _ref1 ? _j < _ref1 : _j > _ref1; i = 0 <= _ref1 ? ++_j : --_j) {
  ...
  chart8.drawChart = function() {
    var data, daysInMonth, firstPeriod, options, originTitle, pieText, pieTooltip, region, secondPeriod, sumValues, _j, _ref1;
  ...
  spark2.drawChart = function() {
    ...
    sumValues = function(year, month, type, origin) {
      ...
      if (H5.Data.region === "Todos") {
        ...
        $.each(H5.DB.occurence.data.regions[H5.Data.region], function(key, reg) {
          var _ref1;
          if ((firstPeriod <= (_ref1 = reg.date) && _ref1 <= secondPeriod) && reg.month === month && (reg.type.indexOf(type) >= 0 || type === "Todos") && (reg.origin.indexOf(origin) >= 0 || origin === "Todos")) {
            return sum++;
          }
        });
      }
  ...
  knob1.drawChart = function() {
    ...
    periodDeforestationRate = function(year, month, type, origin) {
      ...
      sumValues = function(date) {
        var reg, region, sum, _ref1, _ref2, _ref3, _ref4;
        sum = 0;
        if (H5.Data.region === "Todos") {
          for (region in H5.DB.occurence.data.regions) {
            for (reg in H5.DB.occurence.data.regions[region]) {
              ...
              } else if ((date.getFullYear() <= (_ref2 = reg.year) && _ref2 <= date.getFullYear()) && reg.month === date.getMonth() && (reg.type.indexOf(type) >= 0 || type === "Todos") && (reg.origin.indexOf(origin) >= 0 || origin === "Todos")) {
                sum++;
              }
            }
          }
          ...
          for (reg in H5.DB.occurence.data.regions[H5.Data.region]) {
            ...
            } else if ((date.getFullYear() <= (_ref4 = reg.year) && _ref4 <= date.getFullYear()) && reg.month === date.getMonth() && (reg.type.indexOf(type) >= 0 || type === "Todos") && (reg.origin.indexOf(origin) >= 0 || origin === "Todos")) {
              sum++;
            }
          }
        }
  ...
  knob2.drawChart = function() {
    ...
    periodDeforestationRate = function(year, month, type, origin) {
      var curDate, curValue, preDate, preValue, sumValues;
      sumValues = function(date) {
        var reg, region, sum, _ref1, _ref2, _ref3;
        sum = 0;
        if (H5.Data.region === "Todos") {
          for (region in H5.DB.occurence.data.regions) {
            for (reg in H5.DB.occurence.data.regions[region]) {
          ...
          for (reg in H5.DB.occurence.data.regions[H5.Data.region]) {
            reg = H5.DB.occurence.data.regions[H5.Data.region][reg];
            if ((date.getFullYear() <= (_ref3 = reg.year) && _ref3 <= date.getFullYear()) && reg.month === date.getMonth() && (reg.type.indexOf(type) >= 0 || type === "Todos") && (reg.origin.indexOf(origin) >= 0 || origin === "Todos")) {
              sum++;
            }
          }
        }
    ...
    periodDeforestationAvgRate = function(year, month, type, origin) {
      ...
      sumValues = function(firstPeriod, secondPeriod) {
        ...
        if (H5.Data.region === "Todos") {
          ...
          $.each(H5.DB.occurence.data.regions[H5.Data.region], function(key, reg) {
            var _ref1;
            if ((firstPeriod <= (_ref1 = reg.date) && _ref1 <= secondPeriod) && (reg.type.indexOf(type) >= 0 || type === "Todos") && (reg.origin.indexOf(origin) >= 0 || origin === "Todos")) {
              return sum++;
            }
          });
        }
