...
(function() {
  var bool, chart1, chart2, chart5, chart7, chart8, i, k, knob1, knob2, knob3, opt, ref, rest, spark1, spark2,
  ...
  for (i = k = ref = H5.Data.thisYear; ref <= 2004 ? k <= 2004 : k >= 2004; i = ref <= 2004 ? ++k : --k) {
  ...
  H5.DB.occurence.data = {
    init: function() {
      var l, len, ref1, region;
      this.regions = {};
      ref1 = H5.Data.regions;
    ...
    populate: function(id_ocorrencia, region, date, state, type, origin, damageIdentified, institutionLocal, sourceType, periodDay) {
      ...
      if (indexOf.call(H5.Data.regions, region) < 0) {
        region = "Todos";
      }
  ...
  chart1.drawChart = function() {
    var contMes, createTable, data, day, daysInMonth, firstPeriod, indexMes, l, len, m, mes, mesIndex, monthLoop, months, n, options, ref1, secondPeriod, titleChart, titleX, todosMeses;
    createTable = (function(_this) {
      return function(region, type, origin, indexMonth) {
        var day, daysInMonth, firstPeriod, l, ref1, secondPeriod, sum;
        sum = 0;
        ...
        for (day = l = 1, ref1 = daysInMonth; 1 <= ref1 ? l <= ref1 : l >= ref1; day = 1 <= ref1 ? ++l : --l) {
          $.each(H5.DB.occurence.data.regions[region], function(key, reg) {
            var ref2, ref3;
            if (type === "Todos" && origin === "Todos") {
              if ((firstPeriod <= (ref2 = reg.date) && ref2 <= secondPeriod) && reg.day === day) {
                return sum++;
              }
            } else if ((firstPeriod <= (ref3 = reg.date) && ref3 <= secondPeriod) && reg.day === day && (reg.type.indexOf(type) >= 0 || type === "Todos") && (reg.origin.indexOf(origin) >= 0 || origin === "Todos")) {
              return sum++;
            }
          });
          if (!todosMeses) {
    ...
    contMes = 1;
    indexMes = 0;
    for (m = 0, len = monthLoop.length; m < len; m++) {
      mesIndex = monthLoop[m];
      ...
      if (todosMeses) {
        ...
        for (day = n = 1, ref1 = daysInMonth; 1 <= ref1 ? n <= ref1 : n >= ref1; day = 1 <= ref1 ? ++n : --n) {
          data[0] = day;
          data[1] = 0;
          this.data.addRow(data);
        }
  ...
  chart2.drawChart = function() {
    var data, l, m, month, options, ref1, ref2, sumValues;
      ...
      if (H5.Data.region === "Todos") {
        $.each(H5.DB.occurence.data.regions, function(key, region) {
          return $.each(region, function(key, reg) {
            var ref1, ref2;
            if (type === "Todos" && origin === "Todos") {
              if (((firstPeriod <= (ref1 = reg.date) && ref1 <= secondPeriod)) && (reg.month === month)) {
                return sum++;
              }
            } else if ((firstPeriod <= (ref2 = reg.date) && ref2 <= secondPeriod) && reg.month === month && (reg.type.indexOf(type) >= 0 || type === "Todos") && (reg.origin.indexOf(origin) >= 0 || origin === "Todos")) {
              return sum++;
            }
          });
        });
    ...
    this.createDataTable();
    this.data.addColumn("string", "Mês");
    for (i = l = 0, ref1 = this.options.period; 0 <= ref1 ? l < ref1 : l > ref1; i = 0 <= ref1 ? ++l : --l) {
      this.data.addColumn("number", H5.Data.selectedYear - i);
    }
    for (month in H5.Data.months) {
      data = [H5.Data.months[month]];
      month = parseInt(month);
      for (i = m = 1, ref2 = this.options.period; 1 <= ref2 ? m <= ref2 : m >= ref2; i = 1 <= ref2 ? ++m : --m) {
  ...
  chart5.drawChart = function() {
    var allData, data, institution, j, l, len, m, n, options, ref1, ref2, ref3, sumValues, totalReg;
    sumValues = function(institution, region, year, type, origin) {
      ...
      secondPeriod = new Date(year, 11, 31);
      $.each(H5.DB.occurence.data.regions[region], function(key, reg) {
        var l, len, len1, m, ref1, ref2, ref3, ref4, results, results1, t;
        if (reg.institution !== null) {
          if (type === "Todos" && origin === "Todos") {
            if ((firstPeriod <= (ref1 = reg.date) && ref1 <= secondPeriod)) {
              ref2 = reg.institution.split(",");
              results = [];
          ...
          } else if ((firstPeriod <= (ref3 = reg.date) && ref3 <= secondPeriod) && (reg.type.indexOf(type) >= 0 || type === "Todos") && (reg.origin.indexOf(origin) >= 0 || origin === "Todos")) {
            ref4 = reg.institution.split(",");
            results1 = [];
      ...
      if (H5.Data.region === "Todos") {
        $.each(H5.DB.occurence.data.regions, (function(_this) {
          return function(region, reg) {
            ...
            results = [];
            for (j = n = 1, ref3 = _this.options.period; 1 <= ref3 ? n <= ref3 : n >= ref3; j = 1 <= ref3 ? ++n : --n) {
              ...
              results.push(data[j] = data[j] >= 0 ? data[j] + totalReg : totalReg);
            }
            return results;
          };
        })(this));
        ...
        for (j = n = 1, ref3 = this.options.period; 1 <= ref3 ? n <= ref3 : n >= ref3; j = 1 <= ref3 ? ++n : --n) {
  ...
  chart7.drawChart = function() {
    var data, l, options, originTitle, ref1, region, sumValues;
    sumValues = function(region, year, type, origin) {
      ...
      $.each(H5.DB.occurence.data.regions[region], function(key, reg) {
        ...
        } else if ((firstPeriod <= (ref2 = reg.date) && ref2 <= secondPeriod) && (reg.type.indexOf(type) >= 0 || type === "Todos") && (reg.origin.indexOf(origin) >= 0 || origin === "Todos")) {
          return sum++;
        }
      });
      return Math.round((sum * 100) / 100);
    ...
    this.data.addColumn("number", H5.Data.selectedYear);
    for (i = l = 0, ref1 = H5.Data.regions.length; 0 <= ref1 ? l < ref1 : l > ref1; i = 0 <= ref1 ? ++l : --l) {
  ...
  chart8.drawChart = function() {
    var data, daysInMonth, firstPeriod, l, options, originTitle, pieText, pieTooltip, ref1, region, secondPeriod, sumValues;
  ...
  spark1.drawChart = function() {
    var createTable, data, day, daysInMonth, firstPeriod, l, ref1, secondPeriod, value;
    createTable = (function(_this) {
      return function(region, type, origin) {
        var day, dayValue, l, ref1, results;
        dayValue = 0;
        results = [];
        for (day = l = 1, ref1 = daysInMonth; 1 <= ref1 ? l <= ref1 : l >= ref1; day = 1 <= ref1 ? ++l : --l) {
          $.each(H5.DB.occurence.data.regions[region], function(key, reg) {
          ...
          results.push(data[day - 1] = Math.round((data[day - 1] + dayValue) * 100) / 100);
        }
        return results;
      };
    })(this);
  ...
  spark2.drawChart = function() {
    ...
    sumValues = function(year, month, type, origin) {
      ...
      if (H5.Data.region === "Todos") {
        ...
        $.each(H5.DB.occurence.data.regions[H5.Data.region], function(key, reg) {
          var ref1;
          if ((firstPeriod <= (ref1 = reg.date) && ref1 <= secondPeriod) && reg.month === month && (reg.type.indexOf(type) >= 0 || type === "Todos") && (reg.origin.indexOf(origin) >= 0 || origin === "Todos")) {
            return sum++;
          }
        });
      }
  ...
  knob1.drawChart = function() {
    ...
    periodDeforestationRate = function(year, month, type, origin) {
      ...
      sumValues = function(date) {
        var ref1, ref2, ref3, ref4, reg, region, sum;
        sum = 0;
        if (H5.Data.region === "Todos") {
          for (region in H5.DB.occurence.data.regions) {
            for (reg in H5.DB.occurence.data.regions[region]) {
              ...
              if (type === "Todos" && origin === "Todos") {
                if ((date.getFullYear() <= (ref1 = reg.year) && ref1 <= date.getFullYear()) && reg.month === date.getMonth()) {
                  sum++;
                }
              } else if ((date.getFullYear() <= (ref2 = reg.year) && ref2 <= date.getFullYear()) && reg.month === date.getMonth() && (reg.type.indexOf(type) >= 0 || type === "Todos") && (reg.origin.indexOf(origin) >= 0 || origin === "Todos")) {
                sum++;
              }
            }
          }
          ...
          for (reg in H5.DB.occurence.data.regions[H5.Data.region]) {
            ...
            if (type === "Todos" && origin === "Todos") {
              if ((date.getFullYear() <= (ref3 = reg.year) && ref3 <= date.getFullYear()) && reg.month === date.getMonth()) {
                sum++;
              }
            } else if ((date.getFullYear() <= (ref4 = reg.year) && ref4 <= date.getFullYear()) && reg.month === date.getMonth() && (reg.type.indexOf(type) >= 0 || type === "Todos") && (reg.origin.indexOf(origin) >= 0 || origin === "Todos")) {
              sum++;
            }
          }
        }
  ...
  knob2.drawChart = function() {
    ...
    periodDeforestationRate = function(year, month, type, origin) {
      var curDate, curValue, preDate, preValue, sumValues;
      sumValues = function(date) {
        var ref1, ref2, ref3, reg, region, sum;
        sum = 0;
        if (H5.Data.region === "Todos") {
          for (region in H5.DB.occurence.data.regions) {
            for (reg in H5.DB.occurence.data.regions[region]) {
          ...
          for (reg in H5.DB.occurence.data.regions[H5.Data.region]) {
            reg = H5.DB.occurence.data.regions[H5.Data.region][reg];
            if ((date.getFullYear() <= (ref3 = reg.year) && ref3 <= date.getFullYear()) && reg.month === date.getMonth() && (reg.type.indexOf(type) >= 0 || type === "Todos") && (reg.origin.indexOf(origin) >= 0 || origin === "Todos")) {
              sum++;
            }
          }
        }
    ...
    periodDeforestationAvgRate = function(year, month, type, origin) {
      ...
      sumValues = function(firstPeriod, secondPeriod) {
        ...
        if (H5.Data.region === "Todos") {
          ...
          $.each(H5.DB.occurence.data.regions[H5.Data.region], function(key, reg) {
            var ref1;
            if ((firstPeriod <= (ref1 = reg.date) && ref1 <= secondPeriod) && (reg.type.indexOf(type) >= 0 || type === "Todos") && (reg.origin.indexOf(origin) >= 0 || origin === "Todos")) {
              return sum++;
            }
          });
        }
