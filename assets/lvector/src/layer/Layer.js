  ...
  _processFeatures: function(data) {
    ...
    if (this instanceof lvector.PRWSF) {
      ...
      for (i = 0, len = data.features.length; i < len; i++) {
        ...
        for (prop in data.features[i]) {
          if (prop == "geojson") {
            data.features[i].geometry = data.features[i].geojson;
            delete data.features[i].geojson;
          } else
          if (prop != "properties") {
            data.features[i].properties[prop] = data.features[i][prop];
            delete data.features[i][prop];
          }
        }
